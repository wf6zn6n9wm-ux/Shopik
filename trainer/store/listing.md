...
**Встроенных покупок нет.** Приложение в магазинах бесплатное и внутри ничего
не продаёт: подписку человек оформляет на сайте. В анкетах магазинов отвечать
соответственно — App Store: «Does your app contain in-app purchases?» → **No**;
Google Play: «In-app purchases» → **No**, «Contains ads» → **No**.

Пробный период — 14 дней, полный доступ. Дальше подписка оформляется на
`pro-trainer.pro`, а приложение впускает по уже оплаченному логину.

Идентификаторы подписок пригодятся, если встроенные покупки включат обратно
(сборка с ключом `--sell`) — они совпадают с `PLANS` в `index.html`:

Человек платит на сайте гривной: 299 / 749 / 1990 ₴. Долларовые цены ниже — это
цены продуктов в консоли магазина, они нигде не показываются.

| Product ID | Длительность | Цена в магазине |
|---|---|---|
...
- **Клиенты, тренировки, оплаты и заметки** лежат в хранилище приложения на
  устройстве. На сервер уходит их зашифрованная копия (AES-256). Копию можно
  выключить в настройках. В анкетах это «данные, связанные с пользователем,
  не используемые для отслеживания».
- **Ключ от копии.** Пока включено восстановление пароля, ключ хранится и у
  нас — иначе забытый пароль стоил бы тренеру всей базы. Значит, технически
  мы можем копию открыть, и в анкетах это заявляется именно так, без
  формулировок про «нулевое знание». Тренер может выключить восстановление:
  тогда наш экземпляр ключа стирается и копию открывает только его пароль.
- **Данные не передаются третьим сторонам** и не используются для рекламы и
  трекинга. Рекламных SDK в приложении нет.
- **Аккаунт**: почта — как логин. Пароль обязателен, хранится
  хешем с солью (SHA-256). На почту отправляется код для восстановления
  пароля и письма о конце пробного периода.
- **Оплаты**: проходят на сайте через LiqPay — внутри приложения покупок нет.
  Данные карты приложение не видит и не хранит: ему известны только факт
  активной подписки и её срок.
- **Удаление**: тренер может удалить клиента с историей, очистить все данные
  приложения или сохранить резервную копию в файл.

### Что уже заполнено в Google Play Data safety

Записано, чтобы в следующий раз не выводить заново. Ответы должны совпадать
с описанием на странице магазина: расхождение между анкетой и текстом —
повод снять приложение, и проверяется это автоматически.

Общие ответы: данные собираются, третьим лицам **не передаются**, передача
шифруется, обработка **не временная** (копия лежит на сервере, а не проходит
транзитом). Способ создания аккаунта — только «имя пользователя и пароль».
Адрес для удаления аккаунта и данных — `/delete`.

| Тип данных | Обязателен сбор | Цель |
|---|---|---|
| Имя | нет | функции приложения |
| Электронный адрес | **да** | функции приложения |
| Идентификаторы пользователей | **да** | функции приложения |
| Номер телефона | нет | функции приложения |
| Другие сведения (заметки о клиенте) | нет | функции приложения |
| Другие финансовые сведения (оплаты, долги) | нет | функции приложения |
| Данные о физической активности (тренировки, замеры) | нет | функции приложения |
| Фотографии | нет | функции приложения |
| Другие сообщения в приложении (чат поддержки) | нет | связь с разработчиком |
| Идентификаторы устройства | **да** | функции приложения |

«Обязателен» только там, где без данных нет кабинета: логин и привязка
устройств. Всё остальное едет в копии базы, а копию тренер выключает в
настройках — значит сбор можно запретить, и заявлять надо именно так.

**Информацию о здоровье не отмечаем.** Замеры клиента — вес и обхваты —
это отслеживание прогресса, у Google они попадают в «физическую активность».
«Здоровье» там означает медицинские записи и симптомы, а их приложение не
ведёт.

Ничего из этого не отмечено: местоположение, аудио, файлы и документы,
календарь устройства, контакты устройства, действия в приложении, история
веб-поиска, отчёты о сбоях и диагностика. Аналитики и рекламных SDK в
...
Застосунок працює без інтернету: усе лежить на вашому пристрої. Копію бази
можна вмикати й вимикати. Увімкнена — вона шифрується просто на телефоні
(AES-256) і зберігається в нас, щоб база не зникла разом із телефоном.
Нікому третьому ми даних не передаємо — ні рекламі, ні аналітиці.
...
Приложение работает без интернета: всё лежит на вашем устройстве. Копию базы
можно включать и выключать. Включена — она шифруется прямо на телефоне
(AES-256) и хранится у нас, чтобы база не пропала вместе с телефоном.
Никому третьему мы данные не передаём — ни рекламе, ни аналитике.
...
The app works offline: everything lives on your device. The database copy can
be turned on and off. When it is on, the copy is encrypted on the phone itself
(AES-256) and kept with us, so your data does not disappear along with the
phone. We never pass it to anyone else — no ads, no analytics.
...
Aplikacja działa bez internetu: wszystko leży na Twoim urządzeniu. Kopię bazy
można włączać i wyłączać. Włączona — jest szyfrowana już na telefonie
(AES-256) i przechowywana u nas, żeby baza nie zniknęła razem z telefonem.
Nikomu trzeciemu danych nie przekazujemy — ani reklamie, ani analityce.
...
2. Paid Apps Agreement **не нужен**: приложение бесплатное и ничего не
   продаёт. Он понадобится, только если вернут встроенные покупки.
3. Ссылки на «Умови» и «Політику» указать и в метаданных, не только в
   приложении.
4. Собирать нативную оболочку **без ключа `--sell`** — иначе в сборку попадут
   встроенные покупки, которых магазины не ждут и которые нечем оплатить.
   Проверка `trainer/tests/browser.js` следит, что в такой сборке на экране
   подписки не остаётся ни кнопок покупки, ни ссылок наружу.
5. В заметке для ревьюера написать прямо: приложение — часть сервиса,
   подписка оформляется на сайте, в приложении покупок нет. И дать ему
   доступ, иначе он упрётся в тот же экран, что и обычный пользователь.

   Кабинет живёт на устройстве, а не на сервере, поэтому «войти» под
   готовым логином не выйдет — инструкция должна быть такой:

   > 1. Открыть приложение, пройти первый экран.
   > 2. На экране входа нажать «Нет кабинета — создать» и указать логин
   >    `review@pro-trainer.pro` (или тот, который выдали).
   > 3. Профиль → Підписка → «Я вже оплатив» — подписка привяжется к
   >    устройству и станет активной.

   Саму подписку на этот логин выдать заранее — GitHub → Actions →
   «Выдать подписку». После ревью её можно забрать тем же запуском.
