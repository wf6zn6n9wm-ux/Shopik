...
5. В заметке для ревьюера написать прямо: приложение — часть сервиса,
   подписка оформляется на сайте, в приложении покупок нет. Дать тестовый
   логин с активной подпиской, иначе ревьюер упрётся в тот же экран, что и
   обычный пользователь, и не увидит содержимого.
