...
module.exports = async function handler(req, res){
  ...
  /* ─── вимкнути автопродовження ───
     Доступ лишається до кінця оплаченого періоду, гроші більше не
     списуються. Як саме це виконується — залежить від того, хто
     приймав оплату за цю підписку, а не від того, хто приймає її
     зараз: тренер міг оформити передплату через LiqPay ще до того,
     як платежі перейшли на monobank. */
  ...
  /* LiqPay списує сам, автоматично й без нашого крону — його треба
     попередити явно, інакше банк спише ще раз. monobank навпаки:
     жодних автоматичних списань немає, гроші бере лише наш власний
     крон (api/mono.js) — вимкненого autoRenew йому досить, повідомляти
     нікого не треба. */
  if (lic.provider === 'liqpay' && lic.orderId && L.liqpayReady()){
