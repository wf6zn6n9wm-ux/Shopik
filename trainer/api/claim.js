/* «Відновити покупку»: прив'язати ще один пристрій до вже оплаченої
   підписки. Слотів рівно DEVICES — інакше знання чужої пошти давало б
   чужий доступ усьому світу. */
...
module.exports = async function handler(req, res){
  ...
  if (!devices.includes(device)){
    if (devices.length >= L.DEVICES) return L.json(res, 200, {ok: true, active: false, error: 'device_limit', devices: devices.length, limit: L.DEVICES});
    devices.push(device);
