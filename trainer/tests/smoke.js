...
const {T, el, ctx} = require('./app.js').load();
...
part('оплата на сайті');
{
  ...
  /* Ціни в різних валютах навмисно: магазин рахує в доларах і бере свою
     комісію, сайт — у гривнях, бо рахунок виставляє український ФОП.
     Порівнювати їх числом не можна, а сплутати легко — тоді в банк пішло
     б 4.49 замість 299. Тому стежимо, щоб кожне число лишалось у своїй
     валюті, а не за те, яке з них менше. */
  ok('ціни на сайті — цілі гривні', T.PLANS.every(p => Number.isInteger(p.web) && p.web >= 100),
     T.PLANS.map(p => p.web + ' ₴').join(', '));
  ok('ціни магазину — долари з копійками', T.PLANS.every(p => p.price < 100),
     T.PLANS.map(p => '$' + p.price).join(', '));
...
part('чому не вдалося змінити пароль');
{
  /* Найдорожчий екран застосунку: тут людина або повертає базу клієнтів,
     або лишається ні з чим. Провалитись він уміє чотирма способами, а
     писав про це одне й те саме — і найчастіша порада була неправдою:
     квиток одноразовий, повторне натискання не спрацює ніколи. */
  const spent = T.rekeyMsg('bad_ticket');
  const gone  = T.rekeyMsg('empty');
  const net   = T.rekeyMsg('network');
  const http  = T.rekeyMsg('http_500');

  ok('витрачений квиток веде замовити новий код', /овий код|Замовте новий/i.test(spent), spent);
  ok('  і не радить просто спробувати ще раз', !/ще раз/.test(spent), spent);
  ok('відсутню копію називають відсутньою копією', /опі[їя]/.test(gone) && !/квиток|код/i.test(gone), gone);
  ok('обрив зв’язку названий обривом', /зв’язок|зв.язок|Сервер не відповів/i.test(net), net);
  ok('  невідомий код сервера теж не мовчить', http === net, http);
  ok('усі три відповіді різні', new Set([spent, gone, net]).size === 3);
  ok('жодна не порожня', [spent, gone, net].every(x => x && x.length > 20));

  /* Переклад — не косметика: тренер, у якого зламався вхід, читає це
     своєю мовою, і саме тут не можна лишити порожнє місце. */
  const seen = new Set();
  T.LANGS.forEach(l => {
    T.I18n.set(l.code);
    const m = T.rekeyMsg('bad_ticket');
    seen.add(m);
    ok('  ' + l.code + ': текст про квиток є', !!m && m.length > 20, m);
  });
  /* Перевірка мала не помітити, що переклад не вмикається: перший
     варіант звертався до поля, якого в LANGS немає, мова не мінялась, і
     чотири рядки поспіль виявлялись тим самим українським. Тому
     дивимось не на кожен окремо, а на те, що вони різні. */
  ok('  і в кожній мові він свій', seen.size === T.LANGS.length,
     seen.size + ' різних із ' + T.LANGS.length);
  T.I18n.set('uk');
}
...
part('ключ від копії не замінюємо через обрив зв’язку');
{
  const N = T.needsNewCloudKey;
  ok('копію відкрили — нового ключа не треба', N({opened: true, armed: false}) === false);
  ok('ключа немає й сервер мовчить — заводимо свій', N({opened: false, armed: false}) === true);
  /* Ось те, через що ключ губився: свій ключ є, сервер не відповів —
     і застосунок клав на його місце новий випадковий. Копія на інших
     пристроях цього тренера після цього більше нічим не відкривалась. */
  ok('свій ключ є — обрив зв’язку його не замінює', N({opened: false, armed: true}) === false);
  ok('  і при вдалому відкритті теж не замінює', N({opened: true, armed: true}) === false);
}

part('клавіатура не ховає під собою півекрана');
{
  /* Кнопка «Зберегти» жила на дні сторінки, а дно для браузера не
     рухається: клавіатура просто лягає зверху. Тренер бачив форму без
     кнопки й гортав наосліп, шукаючи її.

     Тепер низ відлічується від видимої частини екрана. Рахунок — тут:
     скільки вікна лишилось невидимим, стільки й займає клавіатура. */
  const K = T.kbHeight;
  ok('клавіатури немає — нічого не піднімаємо',
     K({inner: 844, height: 844, offsetTop: 0}) === 0);
  ok('клавіатура на 336 — стільки й повертаємо',
     K({inner: 844, height: 508, offsetTop: 0}) === 336);
  /* Сторінку можна ще й підтягнути вгору — тоді видима частина не
     тільки нижча, а й зсунута. Не врахувати зсув означало підняти
     кнопку вище, ніж треба, і залишити її висіти посеред екрана. */
  ok('зсув видимої частини враховано',
     K({inner: 844, height: 508, offsetTop: 40}) === 296);
  /* Адресний рядок, що ховається під час гортання, теж змінює висоту.
     Від'ємне значення тут означало б, що видимого більше, ніж вікна, —
     такого не буває, але нуль надійніший за мінус у стилях. */
  ok('нижче нуля не опускаємось',
     K({inner: 700, height: 844, offsetTop: 0}) === 0);
}
