...
/* ─── відповідь із Telegram знаходить адресата ───
   Кому пішла відповідь — вирішує перший рядок того повідомлення, на яке
   відповіли. Поки сповіщення було текстом, рядок лежав у text. Щойно ми
   почали слати картинку з підписом, він переїхав у caption — і бот
   перестав упізнавати кабінет у відповіді на власне ж сповіщення.
   Мовчки: він чесно казав «не зрозумів, кому це», а виглядало як
   поламана переписка. */
part('відповідь із Telegram знаходить кабінет');
{
  const CHAT = API('chat.js');
  const reply = (src, mine) => CHAT.addressee({message: {...mine, reply_to_message: src}});

  let r = reply({text: 'trainer@mail.com\n\nне рахує відсоток'}, {text: 'подивіться налаштування'});
  ok('відповідь на текстове сповіщення', r && r.login === 'trainer@mail.com', (r && r.login) || 'нікого');
  ok('  і сам текст відповіді на місці', r && r.text === 'подивіться налаштування', r && r.text);

  r = reply({caption: 'trainer@mail.com\n\nось так виглядає'}, {text: 'натисніть отут'});
  ok('відповідь на сповіщення з картинкою', r && r.login === 'trainer@mail.com', (r && r.login) || 'нікого');

  r = reply({caption: 'trainer@mail.com\n\nось так виглядає'}, {caption: 'дивіться стрілку', photo: [{file_id: 'x'}]});
  ok('  картинкою у відповідь — теж', r && r.login === 'trainer@mail.com', (r && r.login) || 'нікого');
  ok('  підпис до картинки читається як текст', r && r.text === 'дивіться стрілку', r && r.text);

  /* Запасний шлях: сповіщення загубилось, пишемо «пошта: текст». */
  r = CHAT.addressee({message: {text: 'trainer@mail.com: продовжили підписку'}});
  ok('без відповіді — за адресою на початку рядка', r && r.login === 'trainer@mail.com', (r && r.login) || 'нікого');
