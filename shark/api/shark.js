...
//
// Игровая валюта одна — Telegram Stars (⭐). Пополнить набор звёзд можно за
// Telegram Stars, TON или USDT, но зачисляются всегда звёзды: TON и USDT здесь
// только способы оплаты, играть ими нельзя и на баланс они не попадают.
// Выигрыш забирают заявкой — автоматических выплат в приложении нет.
//
...
// ============================================================
// Рулетка: приз — МНОЖИТЕЛЬ к ставке, а не фиксированная сумма. Прежняя
// таблица фиксированных призов давала дому 46.3% — это грабёж при любой
// валюте. Новая настроена на тот же рейк, что и PVP: матожидание игрока
// 0.958, дому 4.2%, каждый четвёртый
...
//  «звёзды с баланса → кейс» не существует: кейс не тратит игровой баланс и
//  не пересекается с ним ни в одной точке.
//
//  У кейса, в отличие от игр, выплата — настоящий подарок со своей
...
// ============================================================
//  ⭐ Telegram Stars — единственная игровая валюта
// ============================================================
// Игровая валюта — Telegram Stars (⭐), целое число. Дробных звёзд не бывает,
// поэтому весь денежный слой — обычные целые: никаких дробей и округлений на
...
module.exports = async (req, res) => {
  try {
    ...
    if (action === 'state') {
      ...
      // заработок с рефералов — только звёзды: старые строки в грн остаются в
      // истории, но в текущий счёт не идут
      ...
      json(res, 200, {
        ...
        isAdmin: IS_ADMIN,
        config: {
          claimMinStars: starInt(CFG.claim_min_stars) || 500,
          claimHours: starInt(CFG.claim_hours) || 24,
          referralSharePercent: REF_PCT,
          referralBonusStars: starInt(CFG.referral_bonus_stars)
        },
        referrals: {
          count: refs.length, earnedStars,
          sharePct: REF_PCT, bonusStars: starInt(CFG.referral_bonus_stars), friends
        },
        catalog: {
    ...
    if (action === 'admin_stats') {
      ...
      const [users, new24h, new7d, active24h] = await Promise.all([
      ...
      // Звёзды целые, поэтому сумма по выборке точна — но выборка ограничена
      // ADMIN_SCAN, и об этом честно сообщается полем scan.
      const bal = await sbGet('shark_users?select=stars_balance&limit=' + ADMIN_SCAN);
      let held = 0;
      bal.forEach((u) => { held += starInt(u.stars_balance); });

      // Оборот за 7 дней: только звёзды. Строки старой экономики в леджере
    ...
    //  Начисляются звёзды — единственная валюта приложения. Чужой баланс из
    ...
    async function resolvePvpRound(round, stale) {
      ...
      // всё в целых звёздах, поэтому банк складывается точно
    ...
    async function bumpStats(tg, d) {
      ...
      // копим чистый выигрыш в звёздах
