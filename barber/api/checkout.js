/* Создание оплаты: отдаём страницу, которая сама отправляет форму в
   LiqPay. Приватный ключ остаётся на сервере — в браузер едет только
   подпись. */
...
module.exports = function handler(req, res){
  ...
  const lang = ['uk', 'ru', 'en'].includes(q.lang) ? q.lang : 'ru';
  ...
  const orderId = 'pb_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8);
...
background:#080A0C;color:#8A9099}</style>
<body>
<p>…</p>
