...
part('телефон не ездит вбок');
{
  /* Раскладку проверяет браузер, а не песочница — здесь стоят два
     правила, без которых кабинет уезжает за правый край на 390px.
     Оба нашлись живьём, после перевода на украинский: подписи стали
     длиннее, и то, что помещалось раньше, помещаться перестало. */
  const css = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8')
    .slice(0, fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8').indexOf('</style>'));

  /* У грид-элементов min-width по умолчанию auto: одна широкая карточка
     растягивает колонку шире экрана, и вбок едет вся страница. */
  const grid = (css.match(/\n\s*\.grid\{[^}]*\}/) || [''])[0];
  ok('колонки сетки умеют сжиматься', /minmax\(0\s*,\s*1fr\)/.test(grid), grid.trim());

  /* Кнопка «Завершити та записати виручку» не помещается в половину
     экрана. Перенос строки — единственный выход: обрезать глагол в
     кнопке, которая записывает деньги, нельзя. */
  const foot = css.slice(css.indexOf('@media (max-width:920px)'));
  ok('подписи кнопок в модалке переносятся',
     /\.modal-foot \.btn\{[^}]*white-space:normal/.test(foot));
  ok('и «отмена» не забирает половину ширины',
     /\.modal-foot \.btn-soft\{[^}]*flex:0/.test(foot));

  /* Демо-услуга с длинным названием ужималась многоточием на телефоне —
     барбер видел бы обрывок вместо примера. */
  const long = T.SERVICE_SEED.filter(s => s.name.length > 20).map(s => s.name);
  ok('названия демо-услуг помещаются в строку', long.length === 0, long.join(', '));
}
