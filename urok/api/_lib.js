/* ──────────────────────────────────────────────────────────────────
   Urok+ · спільне для серверної частини
   ...
   Зроблено за зразком trainer/api — той код уже пройшов бойову
   експлуатацію, і розходитись із ним без причини немає сенсу.
   ...
   Змінні оточення (Vercel → Settings → Environment Variables):
     ...
     PUBLIC_BASE_URL      https://urok.example
   ────────────────────────────────────────────────────────────────── */
...
/* ─────────── тарифи ───────────
   Ціни у гривні й мають збігатися з PRODUCTS у src/10-core.js та
   PLANS у pay.html. LiqPay уміє регулярні списання лише з періодом
   «місяць» або «рік», тому план на 3 місяці продається разовим
   платежем — і на сторінці оплати про це написано прямо. */
const CURRENCY = 'UAH';
const PLANS = {
  monthly:   {id: 'monthly',   months: 1,  uah: 149,  period: 'month'},
  quarterly: {id: 'quarterly', months: 3,  uah: 399,  period: null},
  yearly:    {id: 'yearly',    months: 12, uah: 1490, period: 'year'},
};
...
/* Ім'я змінних залежить від того, під яким префіксом Upstash під'єднали
   до проєкту. Замість того щоб вгадувати одне, беремо перше, що є. */
...
/* ─────────── ліцензія ───────────
   Ключ — нормалізована пошта (той самий логін, що людина ввела на
   сторінці оплати). Пристрої обмежені, інакше знання чужої пошти
   давало б чужу підписку.                                           */
const normLogin = raw => {
...
module.exports = {
  ENV, configured, PLANS, DEVICES, CURRENCY,
