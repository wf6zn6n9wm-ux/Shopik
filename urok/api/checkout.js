/* Створення оплати: віддаємо сторінку, яка сама відправляє форму в
   LiqPay. Приватний ключ лишається на сервері — у браузер їде тільки
   підпис. */
...
module.exports = function handler(req, res){
  ...
  const lang = ['uk', 'ru', 'en'].includes(q.lang) ? q.lang : 'uk';
  ...
  const orderId = 'up_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8);
  ...
  const payload = {
    ...
    amount: plan.uah,
    currency: L.CURRENCY,
    description: 'Urok+ Premium · ' + plan.id,
    order_id: orderId,
    language: lang === 'en' ? 'en' : lang,
    server_url: base + '/api/callback',
    result_url: base + '/paid.html?lang=' + lang,
  ...
  /* Сторінка-перехідник: форма відправляється сама, тому людина бачить
     лише мить із написом і одразу опиняється на сторінці банку. */
  res.setHeader('content-type', 'text/html; charset=utf-8');
  res.setHeader('cache-control', 'no-store');
  res.status(200).send(`<!doctype html>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Urok+</title>
<style>body{margin:0;min-height:100dvh;display:grid;place-items:center;background:#0B0D10;color:#9AA3AE;
font:600 15px/1.5 -apple-system,system-ui,sans-serif}</style>
<body>
<p>${lang === 'ru' ? 'Открываем оплату…' : lang === 'en' ? 'Opening payment…' : 'Відкриваємо оплату…'}</p>
