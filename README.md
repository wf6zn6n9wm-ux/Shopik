...
- **Владелец** заводит **модель** (📷 фото + название) и добавляет **пары по
  размерам** с **себестоимостью за пару** (сразу с доставкой), группирует по
  папкам. Видно наличие: напр. 44 → 2 шт, 45 → 1 шт.
- **Товар покупает управляющая за свои деньги** — себестоимость это её вложение.
- **Управляющая** продаёт: выбирает из списка **модель + размер**, вносит **цену
  продажи** и **зарплату продавцу** → приложение считает прибыль, **остаток −1**.
- Расчёт: **чистая прибыль = продажа − себестоимость − зарплата продавцу**, делится
  **30 % управляющей / 70 % владельцу** (процент **от прибыли**, настраивается 0–100 %).
  Управляющая забирает **вложенную себестоимость назад + её долю**; **зарплата — расход,
  уходит продавцу**; владельцу — **только его доля прибыли**. (Проверка: доли + зарплата = цена продажи.)

## Что видит каждый

- **Управляющая** — вложено сейчас, твоя доля (заработано), выплачено продавцам, пар на
  складе и главное — **«можно забрать сейчас»** (вложенное + доля по непосчитанным продажам).
- **Владелец** — оборот, чистая прибыль, его доля и **«к получению от управляющей»**
  (его % прибыли по непосчитанным продажам).

Кнопка **«Рассчитаться»** на продаже фиксирует, что деньги распределены.

На вкладке **«Итоги»** есть дашборд **«Динамика»** с переключателем **По дням / По
месяцам**: по каждому периоду — продажи, чистая прибыль, твоя доля, зарплата
продавцам и полоска-график (группировка по `sold_at`).
...
create table profit_shops (
  ...
  salary_pct int  default 0,       -- зарплата продавцу, % от цены продажи (авто; расход)
...
# Gravity Defied — мототриал (папка `gravity/`)

**Отдельная игра**, один самодостаточный файл `gravity/index.html`: чистый JS +
canvas, ни одной внешней библиотеки, ни одного запроса в сеть. Открывается
двойным кликом с диска, работает офлайн, разворачивается отдельным
Vercel-проектом (Root Directory: `gravity`) и живёт как Telegram Mini App / PWA.
Файл можно просто переслать — он самодостаточный.

Переосмысление java-игры 2004 года: вид сбоку, белый фон, зелёный проволочный
ландшафт, мотоцикл на двух колёсах и физика, в которой всё решает баланс.

## Как играть

- **↑** газ, **↓** тормоз, **←** / **→** наклон корпуса назад и вперёд.
  **R** — заново, **Esc** — в меню. На телефоне — четыре экранных кнопки.
- Цель — доехать до флага. Время идёт с первой секунды, рекорд по каждой
  трассе хранится в `localStorage`.
- **Коснулся земли головой — перелом шеи**, заезд закончен (как в оригинале).
  Свалился в яму и стоишь без движения 5 секунд — «застрял», тоже с начала.
- 18 трасс: три тура (**Новичок**, **Любитель**, **Профи**) по шесть штук.
  Следующая открывается после финиша предыдущей.

## Как устроено

Трассы **генерируются по номеру** (сид + сложность), поэтому весят ноль и у
всех одинаковые: холмы, трамплины через пропасть, столики, гребёнка, уступы,
затяжные подъёмы и спуски. Крутизна волн ограничена так, чтобы склон нигде
не превращался в стену, а ширина ям — так, чтобы яма перепрыгивалась, не
разгоняясь до предела.

Байк — **две материальные точки** (колёса) на жёсткой раме. Наклон игрока,
отдача двигателя и гашение вращения дают момент; земля отвечает импульсом,
трение качения и юз считаются от времени, а не от кадра. Физика идёт
фиксированным шагом (480 подшагов в секунду) отдельно от отрисовки — картинка
может тормозить, поведение байка от этого не меняется.

## Проверки

`node tests/gravity.js` — генератор трасс и физика в node (ядро вырезается из
html по меткам `//#region ядро`). Кроме очевидного, проверки стерегут три
дефекта, которые уже случались: разгон до 300 м/с на отвесной стенке ямы,
трение, считавшееся на подшаг вместо секунды (байк терял 85 % скорости за
секунду), и голова, проскакивавшая рельеф насквозь без падения.
`node tests/gravity-ui.js` — меню, заезд, рекорды и раскладка в браузере.
