...
# Urok+ — CRM для репетиторів (папка `urok/`)

**Самостійний застосунок** для репетиторів, викладачів і тренерів. Пов'язані
між собою учні, розклад, заняття, домашні завдання, оплати, дохід і
статистика — плюс маркет матеріалів. Живе в папці `urok/`, відкривається за
`/urok/`, з рештою репозиторію не пов'язаний.

Три мови (українська за замовчуванням, російська, англійська), світла й
темна теми, дані в `localStorage`, архітектура підписки готова до StoreKit.
Збірки немає — React і Babel із CDN, як у `trainer/`.

Перевірки: `node urok/tests/i18n.js` і `node urok/tests/smoke.js` (ганяються
в CI на кожну зміну в `urok/`). Подробиці — в [`urok/README.md`](urok/README.md).
