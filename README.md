...
# Найди предметы — игра-искалка (папка `game/`)

Мобильная игра «hidden object»: на детально заполненной сцене нужно отыскать
все спрятанные предметы из списка снизу. Один самодостаточный `index.html`
(чистый JS + CSS, без сборки и зависимостей) — деплоится как отдельный проект
на Vercel (Root Directory: `game`), Shopik / PARA / «Доля» не затрагиваются.

Что внутри:

- **3 уровня-сцены** — «Уютное дупло» (зимний вечер с белками), «Греческий
  дворик» (Санторини), «Кладовка мышей». Каждая сцена — десятки предметов,
  разбросанных поверх тематического фона; часть перекрывает друг друга, чтобы
  усложнить поиск.
- **Геймплей** — тапни предмет из списка внизу, чтобы его найти; счётчик и
  прогресс-бар вверху, промах даёт лёгкий штраф к таймеру.
- **Управление** — панорама картинки пальцем, щипок двумя пальцами (или колесо
  мыши) для приближения, кнопка `⤢` сбрасывает масштаб.
- **Подсказки** — кнопка `🔍` подводит камеру к случайному ненайденному
  предмету и подсвечивает его (3 подсказки на уровень).
- **Прогресс** — звёзды (1–3, зависят от времени, числа подсказок и промахов),
  последовательная разблокировка уровней и общий счётчик хранятся в
  `localStorage`; уровни можно перепроходить.
- **Мелочи** — звуковые эффекты на Web Audio (отключаются в настройках),
  экран победы со звёздами и статистикой, сброс прогресса.

Раскладка предметов детерминированная (seeded random по id сцены) — один и тот
же уровень всегда собран одинаково, это честный пазл, а не случайная россыпь.
