...
- Расчёт: **чистая прибыль = продажа − себестоимость − зарплата**, делится
  **30 % управляющей / 70 % владельцу** (процент **от прибыли**, настраивается 0–100 %).
  Управляющая забирает **вложенную себестоимость назад + зарплату + её долю**;
  владельцу — **только его доля прибыли**. (Проверка: суммы всегда складываются в цену продажи.)

## Что видит каждый

- **Управляющая** — вложено сейчас, заработано (ЗП + доля), зарплата, пар на складе и
  главное — **«можно забрать сейчас»** (вложенное + ЗП + доля по непосчитанным продажам).
