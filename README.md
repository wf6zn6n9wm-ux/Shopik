...
Stars): 10 за первый уровень, дальше рост шагами по пять, 50 за пятидесятый —
всего 1500 за игру. Повторный заезд звёзд не приносит, но рекорд времени
обновляет.

Транспорт открывается по мере прохождения либо покупается за звёзды раньше
срока — всего на весь гараж нужно 1100 звёзд:
...
и у всех одинаковые. Чем выше уровень, тем чаще выпадают тяжёлые секции: на
первых трассах это холмы и столики (пропастей нет вовсе — велосипед бы их не
перепрыгнул), на последних — пропасти, гребёнка и уступы. Крутизна волн
ограничена так, чтобы склон нигде не превращался в стену, а ширина ям — так,
чтобы яму перепрыгивал транспорт, доступный на этом уровне.
