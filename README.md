...
- **↑** газ, **↓** тормоз, **←** / **→** наклон корпуса назад и вперёд.
  **R** — заново, **Esc** — пауза. На телефоне — четыре экранных кнопки.
- Выход из заезда — **кнопка паузы в левом верхнем углу**: «Продолжить»,
  «Заново», «В меню». На паузе время и физика замирают.
- Цель — доехать до флага. Время идёт с первой секунды, рекорд по каждому
  уровню хранится в `localStorage`.
...
- **50 уровней**, каждый следующий сложнее. Следующий открывается после
  прохождения предыдущего.
- Главный экран — **Играть** (сразу текущий уровень), **Уровни**, **Гараж**,
  **Настройки** и **Как играть** с правилами. Сетка уровней и настройки
  спрятаны в разделы, а не висят на первом экране.
- В сетке уровней каждая клетка показывает **профиль своей трассы** целиком —
  видно, что за уровень, не заходя в него. Трассы для превью считаются один
  раз и кэшируются, заезд берёт трассу из того же кэша.
- В настройках **«Уровни»** переключают открытие: «все открыты» (можно
  посмотреть любую трассу) или «по порядку» (обычное открытие прохождением).

## Звёзды и гараж

За **первое** прохождение уровня начисляются звёзды (оформлены как Telegram
Stars): **10 за первый уровень и по 5 за каждый следующий** — всего 255 за
игру. Повторный заезд звёзд не приносит, но рекорд времени обновляет.

Транспорт открывается по мере прохождения либо покупается за звёзды раньше
срока. Цены выше, чем можно накопить одними заездами: покупка — это платный
способ пройти дальше быстрее, а бесплатно всё равно открывается прохождением.

| транспорт | бесплатно с уровня | цена | скорость | тяга |
|---|---|---|---|---|
| Велосипед | 1 | — | 12.5 | 17 |
| Электровелосипед | 10 | 100 ★ | 15.5 | 24 |
| Мотоцикл | 20 | 200 ★ | 19 | 31 |
| Эндуро | 30 | 300 ★ | 22 | 37 |
| Кроссовый | 40 | 500 ★ | 25.5 | 44 |

Транспорт меняет не только вид: у каждого своя тяга, предельная скорость,
колёсная база, радиус колеса, отдача двигателя и упругость приземления.
Кнопка **«Вид»** в меню переключает отрисовку: цветной / силуэт / ретро.
Кнопка **«Транспорт»** переключает режим: «всё открыто» (можно кататься на
чём угодно) или «по уровням» (открывается прохождением и покупкой).

## Звук

У каждого транспорта своя цепочка синтеза, никаких файлов:

- **велосипед** — шелест покрышек, ветер и щелчки трещотки; щелчки это
  узкополосный шум, который прерывает квадратный LFO, а его частота идёт от
  скорости вращения колеса. Под газом (педалями) трещотка стихает, на
  накате — слышна;
- **электровелосипед** — то же плюс тихий вой мотор-колеса;
- **мотоциклы** — две расстроенные пилы с шумом выхлопа и четырьмя
  «передачами»: на каждой обороты растут заново. Эндуро и кроссовый звучат
  выше, кроссовому добавлена квадратная гармоника.

Всё идёт через мягкий фильтр и компрессор. Громкость проверяется не на слух:
`gravity-ui.js` рендерит две секунды каждого транспорта в `OfflineAudioContext`
и сверяет пик и RMS — раньше звук резал уши, теперь за этим следит проверка.
В гараже выбор транспорта проигрывает короткий «рык», чтобы услышать разницу.

## Как устроено

Трассы **генерируются по номеру уровня** (сид + сложность), поэтому весят ноль
и у всех одинаковые. Рельеф набирается из **прямых отрезков с резкими
изломами** — острые пики, узкие щели, уступы, отвесные сбросы, кикеры с
пропастью за ними, — как в оригинале. Гладких волн нет: именно на изломах
байк и приходится вести.

Два правила держат трассы проходимыми:

- **подъём — либо заезжаемый, либо стена.** Всё, на что надо заехать колёсами,
  не круче 55°; всё, что круче 71°, — откровенная стенка ямы, её перепрыгивают.
  Промежуточных «почти стен» нет: именно на них уровень становился
  непроходимым;
- **после каждого спуска идёт ровное дно шире колёсной базы.** Без него байк
  просто заклинивает в «галочке» между изломами — переднее колесо упирается в
  стенку, и выехать нельзя вообще никак.

Байк — **две материальные точки** (колёса) на жёсткой раме. Физика идёт
...
## Почему нужны все четыре кнопки

Игра намеренно не проходится одним газом. За это отвечают четыре механики:

- **отдача двигателя.** Полный газ задирает нос: на ровной прямой мотоцикл
  опрокидывается через 4–6 секунд. Газ надо дозировать и парировать наклоном;
- **байк не выравнивается сам.** Гашение вращения почти убрано — в полёте
  байк сохраняет вращение, и развернуть его под уклон должен игрок;
- **жёсткое приземление.** Если угол байка разошёлся с уклоном больше чем на
  46° и скорость в полёте была выше 6 м/с — байк складывается. Ради этого в
  воздухе и работают наклон, газ (нос вверх) и тормоз (нос вниз);
- **переворот.** Легли на спину — заезд окончен, без всякого касания головой.

Наклон в воздухе работает в полную силу, а на земле вдвое слабее: землёй байк
не перевернуть, но перенести баланс на подъёме или спуске можно. Сам наклон
нарастает за доли секунды — короткое касание даёт лёгкую поправку, удержание
полный момент, иначе байк швыряло бы от каждого нажатия.

Держится это проверками, а не на слово: бот, который **только держит газ**,
проходит 1 уровень из 50 (раньше — 8, а первую четверть игры подряд).

## Проверки

`node tests/gravity.js` — транспорт, экономика звёзд, генератор трасс и физика
в node (ядро вырезается из html по меткам `//#region ядро`). Кроме очевидного,
проверки стерегут три дефекта, которые уже случались: разгон до 300 м/с на
отвесной стенке ямы, трение, считавшееся на подшаг вместо секунды (байк терял
85 % скорости за секунду), и голова, проскакивавшая рельеф насквозь без
падения. `node tests/gravity-ui.js` — меню на 50 уровней, гараж, покупка за
звёзды, заезд, рекорды и раскладка в браузере.
